...
export async function createPoolInteractive(
  ...
  const { tx, pair } = await sarosDLMM.createPairWithConfig({
    tokenBase: {
      mintAddress: tokenBase.mintAddress,
      decimal: tokenBase.decimals,
    },
    tokenQuote: {
      mintAddress: tokenQuote.mintAddress,
      decimal: tokenQuote.decimals,
    },
  ...
  return {
    tx: (tx as unknown) as import("@solana/web3.js").Transaction,
    pair,
    wallet,
    pairAddress,
  };
